...
func create_entities(terrain, diffic: String) -> void:
	...
	for x in map_size:
		for y in map_size:
			temp_data[Vector2(x, y)] = _get_entity_data(terrain[Vector2(x, y)], noise.get_noise_2d(float(x), float(y)))
...
func _get_entity_data(terrain: int, noise_sample: float) -> Entity:
	# grass = 0
