...
func _handle_pitch_to_zoom(event: InputEvent) -> void:
	if event is InputEventPanGesture:
		if abs(event.delta.y) > 0.1:
			_zoom(zoom_step * event.delta.y)

func _handle_pan_gesture(event: InputEvent) -> void:
...
func _handle_middle_mouse(event: InputEvent) -> void:
	if Input.is_mouse_button_pressed(BUTTON_MIDDLE):
		...
		if not event is InputEventMouseMotion:
			init_cam_pos = position
			init_mouse_pos = mouse_pos
		
		# multiply by -1 in order move the camera in the direction of the mouse.
		_set_position(init_cam_pos + (mouse_pos - init_mouse_pos) * -1 * zoom)
